...
const KalosList = () => {
  ...
  useEffect(() => {
    const fetchKalosPokemon = async () => {
      try {
        ...
        // Filter Pokemon with IDs between 650 and 721
        const kalosPokemon = data.results.filter((pokemon) => {
          const pokemonId = parseInt(pokemon.url.split("/")[6]);
          return pokemonId >= 650 && pokemonId <= 721;
        });

        setPokemonList(kalosPokemon);
      } catch (error) {
        console.error("Error fetching Kalos Pokemon list:", error);
      }
    };
  ...
  return (
    <Container fluid>
      <NavBar />
      <h1 className="my-4">Kalos Pokemon List</h1>
      <p>Here is a list of Kalos Pokemon with their sprites:</p>
