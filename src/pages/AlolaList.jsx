...
const AlolaList = () => {
  ...
  useEffect(() => {
    const fetchAlolaPokemon = async () => {
      try {
        ...
        // Filter Pokemon with IDs between 722 and 809
        const alolaPokemon = data.results.filter((pokemon) => {
          const pokemonId = parseInt(pokemon.url.split("/")[6]);
          return pokemonId >= 722 && pokemonId <= 809;
        });

        setPokemonList(alolaPokemon);
      } catch (error) {
        console.error("Error fetching Alola Pokemon list:", error);
      }
    };
  ...
  return (
    <Container fluid>
      <NavBar />
      <h1 className="my-4">Alola Pokemon List</h1>
      <p>Here is a list of Alola Pokemon with their sprites:</p>
